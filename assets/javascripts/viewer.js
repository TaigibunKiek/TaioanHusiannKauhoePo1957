(function () {
  ...
  fetch(`${baseUrl}/chuliau/images.json`)
    ...
    .then(data => {
      ...
      const dataFolderPath = data.folder;
      ...
      const imgSourcesMax = images.length;

      initViewer(imgSources, imgSourcesMin, imgSourcesMax, baseUrl);
    })
    .catch(error => {
      console.error('Error loading image manifest:', error);
    });

  function initViewer(imgSources, imgSourcesMin, imgSourcesMax, baseUrl) {
